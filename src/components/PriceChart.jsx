  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: h, position: 'relative', borderRadius: 8, overflow: 'hidden', background: 'linear-gradient(180deg, rgba(255,255,255,0.01), rgba(255,255,255,0.003))' }}>
      <svg width={w} height={h} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{display:'block'}}>
        <g>
          {tickVals.map((v,i) => {
            ...
            return (
              <g key={i}>
                <line x1={padding.left} x2={w - padding.right} y1={ty} y2={ty} stroke="rgba(255,255,255,0.04)" strokeWidth={1} />
      ...
      {hover && (
        <div style={{
          position:'absolute', 
          left: Math.min(w - 220, Math.max(15, hover.x - 100)), 
          top: 25,
          ...
          <div style={{paddingTop:8, borderTop:'1px solid rgba(255,255,255,0.1)', textAlign:'center'}}>
            <span style={{
              ...
              {Math.abs((hover.item.close - hover.item.open) / hover.item.open * 100).toFixed(2)}%
            </span>
          </div>
        </div>
      )}
